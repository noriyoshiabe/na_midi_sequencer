...
class Editor
  ...
  QUANTIZE_64_3 = Song::TIME_BASE / 12
  ...
  QUANTIZE_64_D = QUANTIZE_64 + QUANTIZE_64 / 2
  ...
  module Command
    ...
    class Tie < Base
      ...
      def execute
        @notes.each { |n| @editor.song.change_gatetime(n, @quantize) }
        @editor.step = @next_step
      end
    ...
    class Move < Base
      ...
      def execute
        @moved.each do |n|
          @editor.song.move_note(n, @step_moved)
    ...
    class Delete < Base
      ...
      def execute
        ...
        @moved.each { |n| @editor.song.move_note(n, -@step_moved) }
      end
